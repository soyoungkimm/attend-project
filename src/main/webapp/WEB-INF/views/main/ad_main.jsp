                    ...
                    <table class="table table-bordered table-responsive-sm mytable" style="width:100%;">
                        ...
                        <c:forEach var="lec" items="${lecturedayList}">
                            <tr>
                                ...
                                <td class="mycolor4">
                                    <c:forEach begin="${lec.normstart}" end="${lec.normstart + lec.normhour - 1}" var="period" varStatus="status">
                                        ${period}
                                ...
                                <td class="mycolor3">
                                    <c:if test="${reststart ne null}">
                                        <c:forEach begin="${lec.reststart}" end="${lec.reststart + lec.resthour - 1}" var="period" varStatus="status">
                                            ${period}
                                            <c:if test="${status.index != status.end}">,</c:if>
                                        </c:forEach>
                                        교시
                                ...
                                <td>
                                    ...
                                    <a href="/lecMove/list.do?sel1=${normyear}&sel2=${lec.term}" class="btn btn-xs btn-outline-primary">보기</a>
                                </td>
                            </tr>
                        </c:forEach>
                        </tbody>
                ...
                <nav>
                    <div id="lec_pagination_div">
                        <ul class='pagination pagination-sm justify-content-center'>
                            ...
                            <%-- 페이지 번호 --%>
                            <c:forEach var="pageNo" begin="${lec_pagination.beginPageNo}" end="${lec_pagination.endPageNo}">
                                <c:choose>
                                    <c:when test="${pageNo == lec_pagination.curPageNo }">
                                        <li class="page-item active"><span class="page-link" style="background-color:steelblue">${pageNo}</span></li>
                                    </c:when>
                                    <c:otherwise>
                                        <li class="page-item"><a class="page-link" onclick="lec_pagination(${pageNo})">${pageNo}</a></li>
                                    </c:otherwise>
                                </c:choose>
                            </c:forEach>
                            ...
                            <c:choose>
                                <c:when test="${lec_pagination.curPageNo < lec_pagination.totalPages }">
                                    <li class="page-item"><a class="page-link" onclick="lec_pagination(${lec_pagination.curPageNo + 1})">▷</a></li>
                                </c:when>
                                <c:otherwise>
                                    <li class="page-item"><a class="page-link">▷</a></li>
                                </c:otherwise>
                            ...
                            <c:choose>
                                <c:when test="${lec_pagination.curBlock < lec_pagination.totalBlocks }">
                                    <li class="page-item"><a class="page-link" onclick="lec_pagination(${lec_pagination.endPageNo + 1})">▶</a></li>
                                </c:when>
                                <c:otherwise>
                                    <li class="page-item"><a class="page-link">▶</a></li>
                                </c:otherwise>
