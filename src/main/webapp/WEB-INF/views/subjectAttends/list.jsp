                                ...
                                <c:forEach var="h" items="${student.h}" varStatus="st" end="${hour * 15}">
                                    <td id="${status.count}^${st.count}">
                                        <c:if test="${h ne 10}">
                                            ...
                                            <c:if test="${h eq null}">text-warning fa fa-question</c:if> fa-1x" onclick="choose1(${status.count},${st.count}, ${student.studentId});" style="cursor:pointer">
                                            </i>
                                        </c:if>
...
<!------------------------------------------------------------------------------>
<script>
    function choose1(rowno, colno, student_id)
    {
        pos1=rowno+"^"+colno;
        pos2=rowno+","+colno;
        document.getElementById( pos1 ).innerHTML="<div style='font-size:10px;'><i class='fa fa-circle-o fa-1x' onclick='choose3("+pos2+",0," + student_id + ");' style='cursor:pointer'></i><br><i class='text-primary fa fa-times-circle-o fa-1x' onclick='choose3("+pos2+",1," + student_id + ");' style='cursor:pointer'></i><br><i class='text-danger fa fa-close fa-1x' onclick='choose3("+pos2+",2," + student_id + ");' style='cursor:pointer'></font>";
    }
    function choose3(rowno, colno, v, student_id)
    {
        $.ajax({
            ...
            success : function(data) {
                ...
                document.getElementById(rowno + "_late").innerHTML = data.late;
                document.getElementById(rowno + "_absent").innerHTML = data.absent;
                document.getElementById(rowno + "_score").innerHTML = data.score;
            },
