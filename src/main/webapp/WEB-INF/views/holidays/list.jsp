                            ...
                            <div class="card-body" style="padding:10px">
                                ...
                                <table class="table table-bordered mytable" style="width:100%;">
                                    ...
                                    <c:forEach var="holiday" items="${holidayList}">
                                        <tr><fmt:parseDate  value="${holiday.holiday}" var="yyyy" pattern="yyyy-MM-dd" />
                                            <td ><fmt:formatDate value="${yyyy}" pattern="yyyy"/></td>
                                            <td >${holiday.holiday} </td>
                                            <td style="text-align:left"><a href="detail.do?id=${holiday.id}">${holiday.reason}</a></td>
                                            <td>
                                                <a href="edit.do?id=${holiday.id}" class="btn btn-xs btn-outline-primary">수정</a>
                                                <a id="del_btn" class="btn btn-xs btn-outline-danger" onClick="if(confirm('삭제할까요?')) location.href='delete.do?id=${holiday.id}';">삭제</a>
                                            </td>
                                        </tr>
                                    </c:forEach>
                                </table>
                                <c:if test="${fn:length(holidayList) != 0}">
                                    <nav>
